...
# PurePursuitDataProcessor와 SimpleComplementaryFuser 클래스
class PurePursuitDataProcessor:
    def __init__(self, alpha=0.0):
        self.heading_fuser = SimpleComplementaryFuser(alpha=alpha)
...
class SimpleComplementaryFuser:
    ...
    def update(self, gyro_rate, gps_heading=None):
        ...
        if gps_heading is not None:
            if abs(predicted_heading - gps_heading) > 180:
                if gps_heading > predicted_heading:
                    predicted_heading += 360
                else:
                    gps_heading += 360
            fused_heading = self.alpha * predicted_heading + (1 - self.alpha) * gps_heading
            self.current_heading = fused_heading % 360
        else:
            self.current_heading = predicted_heading % 360

        return self.current_heading

def quaternion_from_euler(roll, pitch, yaw) -> Quaternion:
    """ZYX 순서의 오일러 각을 쿼터니언으로 변환"""
    cy = math.cos(yaw * 0.5)
    sy = math.sin(yaw * 0.5)
...
class SensorFusionNode(Node):
    def __init__(self):
        ...
        self.processor = PurePursuitDataProcessor(alpha=0.0)
        self.last_lat = None
        self.last_lon = None
    ...
    def imu_callback(self, msg: Imu):
        ...
        if self.last_lat is not None:
            ...
            final_state = self.processor.update(
                imu_gyro_rate_dps=gyro_rate_dps,
                lat=self.last_lat,
                lon=self.last_lon,
                velocity_kmh=self.last_velocity_kmh,
                gps_heading_deg=gps_heading_to_use
            )
            ...
            if final_state["x"] is not None and final_state["heading"] is not None:
                ...
                # ⛔️ 기존의 잘못된 코드 (이 부분을 아래 코드로 대체)
                # yaw_enu = math.radians(90.0 - final_state["heading"])

                # ✅ GPS heading -> ENU yaw 변환 (수정된 공식)
                heading_deg = final_state["heading"]
                yaw_enu_deg = (450.0 - heading_deg) % 360.0 # 올바른 변환 공식
                yaw_enu = math.radians(yaw_enu_deg)
                
                yaw_enu = math.atan2(math.sin(yaw_enu), math.cos(yaw_enu)) # [-pi, pi] 정규화
